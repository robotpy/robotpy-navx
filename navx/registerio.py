...
class RegisterIO:
    ...
    def run(self):
        ...
        try:
            ...
            logger.info(
                "-- Update rate: %shz (%.4fs)", self.update_rate_hz, update_rate
            )
    ...
    def getConfiguration(self):
        ...
        while retry_count < 5 and not success:
            try:
                config = self.io_provider.read(
                    IMURegisters.NAVX_REG_WHOAMI,
                ...
                board_state.selftest_status = config[
                    IMURegisters.NAVX_REG_SELFTEST_STATUS
                ]
                board_state.sensor_status = AHRSProtocol.decodeBinaryUint16(
                    config, IMURegisters.NAVX_REG_SENSOR_STATUS_L
                )
                board_state.gyro_fsr_dps = AHRSProtocol.decodeBinaryUint16(
                    config, IMURegisters.NAVX_REG_GYRO_FSR_DPS_L
                )
                board_state.accel_fsr_g = config[IMURegisters.NAVX_REG_ACCEL_FSR_G]
                board_state.update_rate_hz = config[
                    IMURegisters.NAVX_REG_UPDATE_RATE_HZ
                ]
                board_state.capability_flags = AHRSProtocol.decodeBinaryUint16(
                    config, IMURegisters.NAVX_REG_CAPABILITY_FLAGS_L
                )
    ...
    def getCurrentData(self):
        ...
        ahrspos_update.op_status = curr_data[
            IMURegisters.NAVX_REG_OP_STATUS - first_address
        ]
        ...
        board_state = self.board_state
        board_state.cal_status = curr_data[
            IMURegisters.NAVX_REG_CAL_STATUS - first_address
        ]
        board_state.op_status = curr_data[
            IMURegisters.NAVX_REG_OP_STATUS - first_address
        ]
        ...
        board_state.accel_fsr_g = curr_data[
            IMURegisters.NAVX_REG_ACCEL_FSR_G - first_address
        ]
        board_state.capability_flags = AHRSProtocol.decodeBinaryUint16(
